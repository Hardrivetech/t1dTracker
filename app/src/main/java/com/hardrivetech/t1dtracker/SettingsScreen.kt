...
fun SettingsScreen(db: AppDatabase, prefs: PrefsRepository, onNavigateBack: () -> Unit) {
        ...
        scope.launch {
            val ok = withContext(Dispatchers.IO) {
                try {
                    ...
                    val encrypted = BackupUtil.encryptBackupWithPassword(backupPassword.toCharArray(), json.toByteArray(Charsets.UTF_8))
            ...
            if (ok) Toast.makeText(context, context.getString(R.string.backup_exported), Toast.LENGTH_SHORT).show()
            else Toast.makeText(context, context.getString(R.string.backup_export_failed), Toast.LENGTH_SHORT).show()
        }
    }
    ...
    val openDocumentLauncher = rememberLauncherForActivityResult(
        ...
        scope.launch {
            ...
            if (ok) Toast.makeText(context, context.getString(R.string.import_success), Toast.LENGTH_SHORT).show()
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Column(modifier = Modifier
                ...
                Switch(checked = biometricEnabled, onCheckedChange = { new ->
                    scope.launch {
                        if (new) {
                            ...
                            if (act == null) {
                                Toast.makeText(context, context.getString(R.string.biometric_not_supported), Toast.LENGTH_SHORT).show()
                                return@launch
                            }
                            ...
                            val ok = BiometricAuth.authenticate(act, "Enable biometric", "Confirm to enable biometric lock")
                            if (ok) {
                            ...
                            Toast.makeText(context, context.getString(R.string.biometric_unavailable_disabling), Toast.LENGTH_SHORT).show()
                        }
                    }
                }) }
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                ...
                Switch(checked = allowLegacyPref, onCheckedChange = { new ->
                    if (new) {
                        ...
                        Toast.makeText(context, context.getString(R.string.allow_legacy_encryption_disabled), Toast.LENGTH_SHORT).show()
                    }
                })
            }

            if (showLegacyConfirm) {
                AlertDialog(onDismissRequest = { showLegacyConfirm = false }, title = { Text(stringResource(R.string.allow_legacy_encryption_warning_title)) }, text = { Text(stringResource(R.string.allow_legacy_encryption_warning_text)) }, confirmButton = {
                    TextButton(onClick = {
                        ...
                        if (ok) Toast.makeText(context, "Wrap-key available — you can now migrate via 'Migrate DB'", Toast.LENGTH_LONG).show() else Toast.makeText(context, "Keystore unavailable — wrapped-key encryption may not be possible on this device", Toast.LENGTH_LONG).show()
                    }) { Text(stringResource(R.string.proceed)) }
                }, dismissButton = { TextButton(onClick = { showLegacyConfirm = false }) { Text(stringResource(R.string.cancel)) } })
            }

            OutlinedTextField(value = backupPassword, onValueChange = { backupPassword = it }, label = { Text(stringResource(R.string.backup_password_label)) }, modifier = Modifier.fillMaxWidth())
            ...
            TextButton(onClick = { showRecoveryGuidance = true }) { Text(stringResource(R.string.backup_recommendations_title)) }
            Spacer(modifier = Modifier.height(8.dp))

            Column {
                Button(modifier = Modifier.fillMaxWidth(), onClick = { createDocumentLauncher.launch("backup.t1d") }) { Text(stringResource(R.string.export_backup)) }
                Spacer(modifier = Modifier.height(8.dp))
                Button(modifier = Modifier.fillMaxWidth(), onClick = {
                    scope.launch {
                        val entries = withContext(Dispatchers.IO) { db.insulinDao().getAll() }
                        val file = withContext(Dispatchers.IO) { BackupUtil.createEncryptedBackupFile(context, "backup.t1d", entries, backupPassword.toCharArray()) }
                        if (file != null) {
                            try {
                ...
                Spacer(modifier = Modifier.height(8.dp))
                Button(modifier = Modifier.fillMaxWidth(), enabled = !migrationInProgress, onClick = { showMigrationConfirm = true }) { Text(stringResource(R.string.migrate_db_title)) }

                if (showMigrationConfirm) {
                    AlertDialog(onDismissRequest = { if (!migrationInProgress) showMigrationConfirm = false }, title = { Text(stringResource(R.string.migrate_db_title)) }, text = {
                        Column {
                        ...
                        Row {
                            TextButton(onClick = {
                                scope.launch {
                                    ...
                                    val file = withContext(Dispatchers.IO) {
                                        ...
                                        BackupUtil.createEncryptedBackupFile(context, fileName, entries, backupPassword.toCharArray())
                                    }
                                    if (file != null) {
                                        lastBackupPath = file.absolutePath
                                        Toast.makeText(context, context.getString(R.string.backup_saved, file.absolutePath), Toast.LENGTH_LONG).show()
                                    } else {
                            ...
                            TextButton(onClick = {
                                ...
                                scope.launch {
                                    ...
                                    if (ok) Toast.makeText(context, context.getString(R.string.migration_complete), Toast.LENGTH_LONG).show() else Toast.makeText(context, context.getString(R.string.migration_failed), Toast.LENGTH_LONG).show()
                                }
                            }) { Text(stringResource(R.string.proceed)) }
                        }
                    }, dismissButton = { TextButton(onClick = { if (!migrationInProgress) showMigrationConfirm = false }) { Text(stringResource(R.string.cancel)) } })
                }
                ...
                Button(modifier = Modifier.fillMaxWidth(), enabled = !migrationInProgress && !restoreInProgress, onClick = {
                    scope.launch {
                        ...
                        if (availableBackups.isEmpty()) Toast.makeText(context, context.getString(R.string.no_migration_backups), Toast.LENGTH_SHORT).show() else {
                ...
                if (showRestoreConfirm) {
                    AlertDialog(onDismissRequest = { if (!restoreInProgress) showRestoreConfirm = false }, title = { Text(stringResource(R.string.restore_pre_migration)) }, text = {
                        Column {
                        ...
                        Row {
                            TextButton(onClick = {
                                ...
                                scope.launch {
                                    ...
                                    if (ok) Toast.makeText(context, context.getString(R.string.restore_complete), Toast.LENGTH_LONG).show() else Toast.makeText(context, context.getString(R.string.restore_failed), Toast.LENGTH_LONG).show()
                                }
                            }) { Text(stringResource(R.string.restore_button)) }
                            Spacer(modifier = Modifier.width(8.dp))
                            TextButton(onClick = { if (!restoreInProgress) showRestoreConfirm = false }) { Text(stringResource(R.string.cancel)) }
                        }
                    }, dismissButton = { TextButton(onClick = { if (!restoreInProgress) showRestoreConfirm = false }) { Text(stringResource(R.string.cancel)) } })
                }
                ...
                Spacer(modifier = Modifier.height(8.dp))
                Button(modifier = Modifier.fillMaxWidth(), onClick = { openDocumentLauncher.launch(arrayOf("*/*")) }) { Text(stringResource(R.string.import_backup)) }
                Spacer(modifier = Modifier.height(8.dp))
                Button(modifier = Modifier.fillMaxWidth(), enabled = !migrationInProgress && !restoreInProgress && !rotatingKeyInProgress, onClick = { showRotateConfirm = true }) { Text(stringResource(R.string.rotate_key)) }
            }
        }
    }

    if (showPrivacy) {
        AlertDialog(onDismissRequest = {showPrivacy = false}, title = { Text(stringResource(R.string.privacy_policy_title)) }, text = {
            Column(modifier = Modifier.verticalScroll(rememberScrollState()).padding(8.dp)) {
                Text("Data storage:")
                Text("- All health data (entries, doses) is stored locally on your device using encrypted prefs and Room database. Backups you create are encrypted with a password you provide.")
                Spacer(modifier = Modifier.height(8.dp))
                Text("Telemetry:")
                Text("- Crash and usage telemetry is disabled by default. Enabling 'Allow telemetry' permits anonymous crash and analytics collection to help improve the app. No health data is sent in telemetry.")
                Spacer(modifier = Modifier.height(8.dp))
                Text("Backups & sharing:")
                Text("- Backups are encrypted with PBKDF2-derived AES-GCM using the password you supply. Keep the password safe; it cannot be recovered by the app.")
                Spacer(modifier = Modifier.height(8.dp))
    ...
    if (showRecoveryGuidance) {
        AlertDialog(onDismissRequest = {showRecoveryGuidance = false}, title = { Text(stringResource(R.string.backup_recommendations_title)) }, text = {
            Column(modifier = Modifier.verticalScroll(rememberScrollState()).padding(8.dp)) {
        ...
        }, confirmButton = { TextButton(onClick = { showRecoveryGuidance = false }) { Text(stringResource(R.string.close)) } })
    }

    if (showRotateConfirm) {
        AlertDialog(onDismissRequest = { if (!rotatingKeyInProgress) showRotateConfirm = false }, title = { Text(stringResource(R.string.rotate_key)) }, text = { Text(stringResource(R.string.rotate_key_confirm)) }, confirmButton = {
            TextButton(onClick = {
                showRotateConfirm = false
                scope.launch {
                    rotatingKeyInProgress = true
                    val ok = try { withContext(Dispatchers.IO) { EncryptionUtil.rotateKey(context) } } catch (e: Exception) { false }
                    rotatingKeyInProgress = false
                    if (ok) Toast.makeText(context, context.getString(R.string.rotate_key_success), Toast.LENGTH_LONG).show() else Toast.makeText(context, context.getString(R.string.rotate_key_failed), Toast.LENGTH_LONG).show()
                }
            }) { Text(stringResource(R.string.proceed)) }
        }, dismissButton = { TextButton(onClick = { if (!rotatingKeyInProgress) showRotateConfirm = false }) { Text(stringResource(R.string.cancel)) } })
    }
}
